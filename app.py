...
SHORT_COOLDOWN    = 600      # cooldown mínimo por setup idêntico (10 min)
M1_COOLDOWN       = 300      # cooldown M1 por par+tipo (5 min)
HTF_COOLDOWN      = 1800     # cooldown HTF (30 min)
HEDGE_COOLDOWN    = 3600      # cooldown HEDGE por par (60 min)
MAX_OPEN_TRADES   = 3        # máximo de trades abertos SIMULTÂNEOS no total
MAX_OPEN_PER_SYM  = 1        # máximo de trades abertos por símbolo
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # ── API: status geral ────────────────────────────────
            ...
            news_out = []
            for n in []:  # notícias removidas — usar /calendario
                news_out.append({
                    "title":  n.get("title",""),
                    "source": n.get("source",""),
                    "emoji":  n.get("emoji",""),
        ...
        # ── API: performance real da corretora ───────────────
            ...
            try:
                ...
                pnl_bot= sum(s.get("pnl_brl",0) for s in wins+losses)
                sym_stats = {}
                for sym in SYMBOLS:
                    ss  = [s for s in sinais if s.get("symbol")==sym]
                    sw  = [s for s in ss if s.get("status")=="win"]
                    sl2 = [s for s in ss if s.get("status")=="loss"]
                    tf2 = len(sw)+len(sl2)
                    sym_stats[sym] = {
                        "wins": len(sw), "losses": len(sl2),
                        "wr": round(len(sw)/tf2*100,1) if tf2>0 else 0,
                        "pnl_brl": round(sum(s.get("pnl_brl",0) for s in sw+sl2),2)
                    }
                payload = {
                    "ok": True,
                    "deposito_brl": dep_total,
                    "saldo_usd":    round(saldo_usd,4),
                    "cripto_brl":   round(cripto_brl,2),
                    "manual_brl":   round(manual_brl,2),
        ...
        # ── API: sinais recentes ──────────────────────────────
            ...
            sinais = memory.get("signals", [])
            body   = json.dumps({"ok":True,"signals":sinais[-100:]},
                                ensure_ascii=False).encode()
            self._respond(200, "application/json", body)
            return

        # ── API: executa comando do bot via URL ───────────────
            ...
            if cmd:
                threading.Thread(
                    target=handle_command, args=(cmd, CHAT_ID), daemon=True
                ).start()
                body = json.dumps({"ok":True,"cmd":cmd}).encode()
            else:
                body = json.dumps({"ok":False,"error":"sem cmd"}).encode()
            self._respond(200, "application/json", body)
            return
            ...
            try:
                ...
                if not cmd or not cmd.startswith("/"):
                    self._respond(400, "application/json",
                                  json.dumps({"ok":False,"error":"cmd inválido"}).encode())
                    return
                # Executa no handle_command usando CHAT_ID padrão
                # Captura a resposta interceptando send_telegram temporariamente
                _responses = []
                _orig = globals().get("send_telegram")
                def _capture(msg, chat_id=None, **kw):
                    _responses.append(msg)
                import builtins
                # Patch temporário
                import sys
                mod = sys.modules[__name__]
                _orig_st = getattr(mod, "send_telegram", None)
                setattr(mod, "_send_telegram_real", _orig_st)
                setattr(mod, "send_telegram", _capture)
                try:
                    handle_command(cmd, CHAT_ID)
                except Exception as e:
                    _responses.append(f"Erro: {e}")
                finally:
                    setattr(mod, "send_telegram", _orig_st)
                result = "\n\n".join(_responses) or f"Comando {cmd} executado."
                resp = json.dumps({"ok":True,"result":result},ensure_ascii=False).encode()
...
MIN_RR   = 2.0   # RR mínimo OBRIGATÓRIO — abaixo disso descarta o sinal
                  # Matemática: com WR=50% e RR=2, lucro líquido = +50%/trade
                  # Com RR=1 e WR=50%, resultado = zero (paga fees e perde)
MIN_CONF = 2     # confirmações mínimas de timeframe para abrir ordem
                  # Ex: M1 UP + M5 UP = 2 confirmações → abre
                  #     M1 UP + M5 NEUTRAL = 1 confirmação → ignora
...
def apply_rr(entry, stop, target, direction):
    """
    Valida o alvo técnico contra o RR mínimo.
    Se o alvo técnico já satisfaz MIN_RR → mantém (respeita estrutura).
    Se for insuficiente → projeta o mínimo necessário para ser lucrativo.
    Nunca coloca alvo do lado errado.
    """
    risco = abs(entry - stop)
    if risco == 0: return target
    # Alvo do lado errado → corrige para mínimo
    if direction == "up"   and target <= entry: return entry + risco * MIN_RR
    if direction == "down" and target >= entry: return entry - risco * MIN_RR
    # Alvo insuficiente → projeta o mínimo
    rr_atual = abs(target - entry) / risco
    if rr_atual < MIN_RR:
        return (entry + risco * MIN_RR if direction == "up"
                else entry - risco * MIN_RR)
    return target
...
# ═══════════════════════════════════════════════════════════════
#  HEDGE — ORDENS GRANDES (alvos H4/H1/M15, RR >= HEDGE_MIN_RR)
#
#  Lógica: abre LONG e SHORT simultaneamente (straddle) a partir do
#  preço atual, com alvos nos níveis de estrutura M15/H1/H4 mais
#  próximos em cada direção. Para onde o preço for, uma das pernas
#  alcança o alvo grande enquanto a outra é stopada (1 risco fixo).
#
#  Risco: 1   |   Retorno: >= HEDGE_MIN_RR (>=3)
#  Stops: técnicos (topo/fundo M15 mais próximo em cada direção)
# ═══════════════════════════════════════════════════════════════
HEDGE_MIN_RR    = 3.0     # RR mínimo exigido para abrir HEDGE
HEDGE_QTY_MULT  = 1.0     # lote mínimo padrão — já são 2 ordens simultâneas (long+short)
HEDGE_MIN_PROFIT_USD = 1.0  # alvo de cada perna deve valer >= US$1 de lucro no lote

def _m15_technical_stop(candles_m15, direction, cur, min_wave):
    """Stop técnico baseado em topos/fundos do M15 (mais largo que M1)."""
    h, l = find_pivots(candles_m15, 2)
    if direction == "up":
        fundos = [v for _, v in l if v < cur * 0.999]
        if not fundos: return cur * (1 - 0.012)   # fallback 1.2%
        return max(fundos) * 0.998
    else:
        topos = [v for _, v in h if v > cur * 1.001]
        if not topos: return cur * (1 + 0.012)
        return min(topos) * 1.002

def hedge_entries(candles_m15, candles_h1, candles_h4, min_wave, qty=0):
    """
    Detecta par de ordens HEDGE (long + short simultâneos) com alvos
    nos níveis de estrutura M15/H1/H4 — RR >= HEDGE_MIN_RR de cada lado,
    e lucro projetado no alvo >= HEDGE_MIN_PROFIT_USD (no lote `qty`).

    Retorna (long_, short_) — qualquer um pode ser None se não atingir
    o RR mínimo ou o lucro mínimo nessa direção.
    """
    if len(candles_m15) < 12: return None, None
    cur = candles_m15[-1]["close"]

    # Reúne níveis de resistência/suporte de M15, H1 e H4
    res_levels, sup_levels = [], []
    for tf_candles, n_piv in [(candles_m15, 2), (candles_h1, 3), (candles_h4, 2)]:
        if len(tf_candles) < 10: continue
        h, l = find_pivots(tf_candles, n_piv)
        res_levels += [v for _, v in h[-6:]]
        sup_levels += [v for _, v in l[-6:]]

    if not res_levels or not sup_levels: return None, None

    # ── LONG: alvo na resistência mais próxima ABOVE cur ─────────
    alvos_acima = sorted([r for r in res_levels if r > cur * 1.001])
    long_ = None
    if alvos_acima:
        alvo_l = alvos_acima[0]
        stop_l = _m15_technical_stop(candles_m15, "up", cur, min_wave)
        risco_l = cur - stop_l
        if risco_l > 0:
            rr_l = (alvo_l - cur) / risco_l
            if rr_l >= HEDGE_MIN_RR:
                # busca alvo mais distante se múltiplos níveis empilhados oferecerem RR ainda maior
                for alt in alvos_acima[1:]:
                    rr_alt = (alt - cur) / risco_l
                    if rr_alt >= HEDGE_MIN_RR: alvo_l = alt
                    else: break
                long_ = _build_signal(cur, stop_l, alvo_l, "up", "HEDGE_LONG",
                                       wave_start=stop_l, wave_end=alvo_l)
                if long_:
                    long_["rr"] = max(long_["rr"], HEDGE_MIN_RR)
                    if qty and abs(long_["alvo"] - long_["entry"]) * qty < HEDGE_MIN_PROFIT_USD:
                        long_ = None

    # ── SHORT: alvo no suporte mais próximo BELOW cur ────────────
    alvos_abaixo = sorted([s for s in sup_levels if s < cur * 0.999], reverse=True)
    short_ = None
    if alvos_abaixo:
        alvo_s = alvos_abaixo[0]
        stop_s = _m15_technical_stop(candles_m15, "down", cur, min_wave)
        risco_s = stop_s - cur
        if risco_s > 0:
            rr_s = (cur - alvo_s) / risco_s
            if rr_s >= HEDGE_MIN_RR:
                for alt in alvos_abaixo[1:]:
                    rr_alt = (cur - alt) / risco_s
                    if rr_alt >= HEDGE_MIN_RR: alvo_s = alt
                    else: break
                short_ = _build_signal(cur, stop_s, alvo_s, "down", "HEDGE_SHORT",
                                        wave_start=stop_s, wave_end=alvo_s)
                if short_:
                    short_["rr"] = max(short_["rr"], HEDGE_MIN_RR)
                    if qty and abs(short_["alvo"] - short_["entry"]) * qty < HEDGE_MIN_PROFIT_USD:
                        short_ = None
...
def analyze_symbol(symbol):
    ...
    c15 = get_candles(symbol, "15m", 80)   # base p/ HEDGE
    ...
    # 5. ALTO LUCRO ($1-$5) — stop M1, alvo H1/H4/D1
    c1d = get_candles(symbol, "D", 30) if cm else []
    entries_hp  = high_profit_entries(symbol, cm or [], c1h or [], c4h or [], c1d or [])

    # 5. HEDGE — long+short simultâneos, alvos M15/H1/H4, RR >= HEDGE_MIN_RR
    hedge_long, hedge_short = hedge_entries(c15 or [], c1h or [], c4h or [],
                                             cfg["min_wave"], qty=cfg["qty"]) if c15 else (None, None)
    hedge = (hedge_long, hedge_short) if (hedge_long or hedge_short) else None
...
def _place_order(sym, entry_dict, data, ts, modo, qty=None):
    ...
    qty = qty if qty is not None else SYMBOLS[sym]["qty"]
    side_by = "Buy" if dir_ == "up" else "Sell"
    res = order_futures(sym, side_by, qty, sl=sp, tp=tp)
    bi  = ""
...
def fire_signal(data):
    ...
    # ── ALTO LUCRO ($1-$5): formatação especial ──────────────
    if data.get("hp") and data.get("entry"):
        entry = data["entry"]
        dir_  = entry["direcao"]
        ...
        rr    = entry["rr"]
        lucro = entry.get("lucro_usd_proj", 0)
        risco = entry.get("risco_usd_proj", 0)
        lbl   = entry.get("hp_tf", "HTF")
        tipo  = entry.get("tipo","")
        r     = _place_order(sym, entry, data, ts, modo)
        if not r: return
        sinal, bi_line = r
        rate = get_usd_brl()
        lucro_brl = lucro * rate
        caption = (
            f"💰 <b>ALTO LUCRO — {acao}</b> — {sym}\n"
            f"📌 <code>{tipo}</code> [{lbl}]\n"
            f"💵 Lucro projetado: <b>≈${lucro:.2f} (R${lucro_brl:.2f})</b>\n"
            f"⚠️ Risco: ≈${risco:.2f} | RR 1:{rr}\n"
            f"💰 ${ep:,.4f}  🛑 ${sp:,.4f}  🎯 ${tp:,.4f}\n"
            f"🕐 Stop técnico M1 · Alvo {lbl}\n"
            f"⏰ {ts} (UTC-3)" + ASSINATURA)
        candles_chart = get_candles(sym, "1h", 80)
        _enviar_com_chart(caption, entry, candles_chart or [], lbl)
        save_memory()
        return

    # ── HEDGE: ordens grandes long+short, alvos H4/H1/M15 ────
    hedge = data.get("hedge")
    if hedge:
        long_, short_ = hedge
        if long_ or short_:
            qty_hedge = SYMBOLS[sym]["qty"]
            r_l = _place_order(sym, long_,  data, ts, modo, qty=qty_hedge) if long_ else None
            r_s = _place_order(sym, short_, data, ts, modo, qty=qty_hedge) if short_ else None
            bl  = r_l[1] if r_l else ("❌" if long_ else "—")
            bs  = r_s[1] if r_s else ("❌" if short_ else "—")
            ep  = (long_ or short_)["entry"]
            cap_lines = [f"🛡️ <b>HEDGE</b> — {sym}",
                          f"📐 Long+Short simultâneos · alvos M15/H1/H4",
                          f"💰 Entrada: <b>${ep:,.4f}</b>",
                          f"⚖️ Lote: {qty_hedge} (mínimo padrão)"]
            if long_:
                lucro_l = abs(long_['alvo']-long_['entry'])*qty_hedge
                cap_lines.append(f"📈 Long  SL:${long_['stop']:,.4f}  TP:${long_['alvo']:,.4f}  RR 1:{long_['rr']}  (≈+${lucro_l:.2f})")
            if short_:
                lucro_s = abs(short_['alvo']-short_['entry'])*qty_hedge
                cap_lines.append(f"📉 Short SL:${short_['stop']:,.4f}  TP:${short_['alvo']:,.4f}  RR 1:{short_['rr']}  (≈+${lucro_s:.2f})")
            cap_lines.append(bl); cap_lines.append(bs)
            cap_lines.append(f"⏰ {ts} (UTC-3)" + ASSINATURA)
            caption = "\n".join(cap_lines)
            tf_ref = (long_ or short_)
            candles_chart = get_candles(sym, "15m", 80)
            _enviar_com_chart(caption, tf_ref, candles_chart or [], "HEDGE")
            save_memory()
            return
...
# NOTÍCIAS REMOVIDAS — substituído por Calendário Econômico

# ═══════════════════════════════════════════════════════════════
#  CALENDÁRIO ECONÔMICO — Investing.com scraping
#  Filtra apenas eventos de ALTA PRIORIDADE relacionados
#  aos pares operados pelo bot.
#  Alerta automático 15 min antes do evento.
# ═══════════════════════════════════════════════════════════════

# Moedas relacionadas aos pares operados
CAL_CURRENCIES = {"USD","BTC","ETH","SOL","XRP","BNB","DOGE","ADA",
                  "AVAX","DOT","LINK","LTC","ATOM","NEAR","APT",
                  "SUI","OP","ARB","TRX","TON","PEPE","EUR","GBP","JPY","CNY"}

# Palavras-chave de eventos que afetam cripto
CAL_KEYWORDS = [
    "fed","fomc","federal reserve","interest rate","cpi","ppi","gdp","nfp",
    "non-farm","inflation","powell","ecb","rate decision","rate hike","rate cut",
    "sec","cftc","bitcoin","crypto","blockchain","treasury","yield","unemployment",
    "retail sales","ism","pce","jobs","employment","lagarde","boj","bank of japan",
    "bank of england","boe","pboc","china","us dollar","dxy",
]

CAL_ALERT_MINUTES = 15   # alerta X minutos antes
_cal_cache   = {"events": [], "ts": 0}
_cal_alerted = set()     # ids já alertados
CAL_INTERVAL = 3600      # recarrega calendário a cada 1h

def _fetch_calendar_investing():
    """Busca calendário econômico da semana via Investing.com."""
    try:
        import xml.etree.ElementTree as ET
        from datetime import datetime, timedelta
        import pytz

        url = "https://sslecal2.investing.com/rss"
        r   = requests.get(url, timeout=12,
                           headers={"User-Agent": "Mozilla/5.0 TronBot/3.0",
        ...
        for item in root.iter("item"):
            ...
            t_low = title.lower()
            d_low = desc.lower()
            relevant = any(kw in t_low or kw in d_low for kw in CAL_KEYWORDS)
            if not relevant: continue

            # Parse da data
            try:
                from email.utils import parsedate_to_datetime
                dt_utc = parsedate_to_datetime(pubdate)
                dt_br  = dt_utc.astimezone(tz_br)
            except Exception:
                dt_br = now

            # Só eventos da semana atual
            if abs((dt_br - now).days) > 7: continue

            events.append({
                "id":    hash(title+pubdate),
                "title": title,
                "link":  link,
                "desc":  desc[:120],
                "dt":    dt_br,
                "dt_str": dt_br.strftime("%d/%m %H:%M"),
                "priority": "high",
            })
        ...
        print(f"[CAL] Investing.com: {e}")
        return _fetch_calendar_fallback()

def _fetch_calendar_fallback():
    """Fallback: gera eventos macro padrão da semana com base em datas fixas."""
    from datetime import datetime, timedelta
    import pytz
    tz_br = pytz.timezone("America/Sao_Paulo")
    now   = datetime.now(tz_br)

    # Segunda-Sexta desta semana
    monday = now - timedelta(days=now.weekday())
    events_template = [
        (0, 9, 30,  "USD — ISM Manufacturing PMI"),
        (1, 10, 0,  "USD — JOLTS Job Openings"),
        (2, 10, 15, "USD — ADP Non-Farm Employment"),
        (2, 14, 0,  "USD — FOMC Minutes"),
        (3, 9, 30,  "USD — Initial Jobless Claims"),
        (3, 9, 30,  "USD — CPI (Inflation)"),
        (4, 9, 30,  "USD — Non-Farm Payrolls (NFP)"),
        (4, 9, 30,  "USD — Unemployment Rate"),
    ]
    events = []
    for (day_off, h, m, title) in events_template:
        dt = (monday + timedelta(days=day_off)).replace(
            hour=h, minute=m, second=0, microsecond=0)
        if (dt - now).total_seconds() < -3600: continue  # passados há +1h ignorar
        events.append({
            "id":    hash(title+str(dt)),
            "title": title,
            "link":  "https://investing.com/economic-calendar/",
            "desc":  "Evento macro de alta prioridade — impacto em cripto e forex",
            "dt":    dt,
...
def fetch_calendar(force=False):
    global _cal_cache
    if not force and time.time() - _cal_cache["ts"] < CAL_INTERVAL:
        return _cal_cache["events"]
    events = _fetch_calendar_investing()
    _cal_cache = {"events": events, "ts": time.time()}
    return events

def check_calendar_alerts():
    """Verifica se algum evento está a ≤15 min e dispara alerta."""
    global _cal_alerted
    try:
        ...
        for ev in events:
            eid  = ev["id"]
            if eid in _cal_alerted: continue
            diff = (ev["dt"] - now).total_seconds()
            if 0 < diff <= CAL_ALERT_MINUTES * 60:
                _cal_alerted.add(eid)
                mins = int(diff // 60)
                send_telegram(
                    f"⚠️ <b>CALENDÁRIO ECONÔMICO — {mins} min</b>\n"
                    f"🗓 <b>{ev['title']}</b>\n"
                    f"⏰ {ev['dt_str']} (BRT)\n"
                    f"📝 {ev.get('desc','')}\n"
                    f"⚡ <i>Possível volatilidade nos pares operados!</i>\n"
                    f"🔗 <a href=\"{ev['link']}\">Ver calendário</a>")
    except Exception as e:
        print(f"[CAL ALERT] {e}")

def send_calendar_telegram(chat_id=None):
    """Envia calendário da semana via Telegram."""
    try:
        import pytz
        from datetime import datetime
        tz_br = pytz.timezone("America/Sao_Paulo")
    ...
    if not events:
        send_telegram("📅 Nenhum evento econômico encontrado para esta semana.", chat_id)
        return

    futuros = [e for e in events if now is None or (e["dt"] - now).total_seconds() > -3600]
    if not futuros:
        send_telegram("📅 Nenhum evento futuro esta semana.", chat_id); return

    lines = [f"📅 <b>Calendário Econômico — Semana</b>",
             f"⚡ Alta prioridade | impacto em cripto & forex",
             f"{'─'*28}"]

    for ev in futuros[:12]:
        if now:
            diff = (ev["dt"] - now).total_seconds()
            if diff < 0:
                when = "✅ Ocorreu"
            elif diff < 3600:
                when = f"⚠️ Em {int(diff//60)}min"
            elif diff < 86400:
                when = f"🕐 Hoje {ev['dt'].strftime('%H:%M')}"
            else:
                when = f"📆 {ev['dt_str']}"
        else:
            when = ev["dt_str"]
    ...
    lines.append(f"\n{'─'*28}\n🔗 <a href='https://investing.com/economic-calendar/'>Calendário completo</a>")
    send_telegram("\n".join(lines), chat_id)

def calendar_loop():
    """Verifica alertas de calendário a cada 5 minutos."""
    time.sleep(90)
    while True:
        try:
...
# ═══════════════════════════════════════════════════════════════
#  ORDENS DE ALTO LUCRO — $1 a $5 por trade
#  Stop: topo/fundo técnico M1 (igual às demais ordens)
#  Alvo: estrutura H1, H4 ou D1 — fica aberta por mais tempo
#  Critério de entrada: RR mínimo para atingir $1 de lucro líquido
#  Lógica separada, não interfere nas ordens existentes
# ═══════════════════════════════════════════════════════════════

HIGH_PROFIT_MIN_USD = 1.0    # lucro mínimo alvo em USD
HIGH_PROFIT_MAX_USD = 5.0    # lucro máximo por ordem
HIGH_PROFIT_RR_MIN  = 2.0    # RR mínimo obrigatório
HIGH_PROFIT_COOLDOWN= 3600   # 1h de cooldown por símbolo
_hp_last_signal     = {}     # sym → timestamp último sinal HP

def _hp_technical_stop_m1(candles_m1, direction, cur):
    """Stop técnico no topo/fundo M1 mais próximo — mesmo critério das demais ordens."""
    h, l = find_pivots(candles_m1, SWING_M1)
    if direction == "up":
        fundos = [v for _, v in l if v < cur * 0.9998]
        return max(fundos) * 0.9993 if fundos else cur * 0.997
    else:
        topos = [v for _, v in h if v > cur * 1.0002]
        return min(topos) * 1.0007 if topos else cur * 1.003

def _hp_find_htf_target(candles_htf, direction, cur, label="HTF"):
    """Encontra alvo de estrutura no timeframe maior."""
    if not candles_htf or len(candles_htf) < 10: return None, None
    h, l = find_pivots(candles_htf, 3)
    if direction == "up":
        alvos = sorted([v for _, v in h if v > cur * 1.003])
        if not alvos: return None, None
        return alvos[0], label   # primeira resistência acima
    else:
        alvos = sorted([v for _, v in l if v < cur * 0.997], reverse=True)
        if not alvos: return None, None
        return alvos[0], label   # primeiro suporte abaixo

def high_profit_entries(symbol, candles_m1, candles_h1, candles_h4, candles_d1):
    """
    Detecta entradas com alvo de $1-$5 de lucro líquido.
    Stop: topo/fundo M1 técnico.
    Alvo: estrutura H1 → H4 → D1 (prioridade para o maior alvo possível).
    Só dispara se o lucro projetado (pontos × qty) ≥ $1.
    """
    cfg = SYMBOLS.get(symbol, {})
    qty = cfg.get("qty", 0)
    if not qty or not candles_m1 or len(candles_m1) < 10:
        return []

    # Cooldown: 1h entre sinais HP por símbolo
    if time.time() - _hp_last_signal.get(symbol, 0) < HIGH_PROFIT_COOLDOWN:
        return []
    ...
    for direction in ("up", "down"):
        stop = _hp_technical_stop_m1(candles_m1, direction, cur)
        risco_pts = abs(cur - stop)
        if risco_pts == 0: continue

        # Tenta H1, H4, D1 — pega o alvo com maior lucro dentro de $1-$5
        for (candles_htf, tf_label) in [
            (candles_h1, "H1"),
            (candles_h4, "H4"),
            (candles_d1, "D1"),
        ]:
            alvo, lbl = _hp_find_htf_target(candles_htf, direction, cur, tf_label)
            if not alvo: continue

            # Verifica direção válida
            if direction == "up"   and alvo <= cur: continue
            if direction == "down" and alvo >= cur: continue

            lucro_pts = abs(alvo - cur)
            lucro_usd = lucro_pts * qty
            risco_usd = risco_pts * qty
            rr        = lucro_pts / risco_pts

            # Critérios: lucro $1-$5 E RR mínimo 2.0
            if lucro_usd < HIGH_PROFIT_MIN_USD: continue
            if lucro_usd > HIGH_PROFIT_MAX_USD:
                # Reduz alvo para não ultrapassar $5
                alvo = (cur + HIGH_PROFIT_MAX_USD / qty) if direction == "up" \
                       else (cur - HIGH_PROFIT_MAX_USD / qty)
                lucro_usd = HIGH_PROFIT_MAX_USD
                rr = abs(alvo - cur) / risco_pts

            if rr < HIGH_PROFIT_RR_MIN: continue

            tipo = f"HP_{lbl}_{'LONG' if direction=='up' else 'SHORT'}"
            s = _build_signal(cur, stop, alvo, direction, tipo,
                              wave_start=stop, wave_end=alvo)
            if s:
                s["lucro_usd_proj"] = round(lucro_usd, 2)
                s["risco_usd_proj"] = round(risco_usd, 2)
                s["hp_tf"]          = lbl
                results.append(s)
                break  # pega só o melhor TF por direção
...
def handle_command(text, chat_id):
    ...
    if cmd in ("/help", "/start", "/commands"):
        ...
        send_telegram(
            ...
            "🛡️ <b>HEDGE (alvos H4/H1/M15, RR>=" + str(HEDGE_MIN_RR) + "):</b>\n"
            "/hedge → analisa posições HEDGE abertas\n\n"
            "⚙️ <b>RISCO:</b>\n"
            f"/risco → parâmetros (RR:{MIN_RR} Max:{MAX_OPEN_TRADES} trades)\n\n"
            "📰 <b>CALENDÁRIO ECONÔMICO:</b>\n"
            "/calendario       → eventos da semana\n"
            "/cal              → atalho rápido\n"
            "⚠️ Alerta automático 15min antes de eventos\n\n"
    ...
    # ── HEDGE — posições abertas do tipo HEDGE_LONG/HEDGE_SHORT ──
    elif cmd == "/hedge":
        abertos = [s for s in memory.get("signals", [])
                   if s.get("status") == "aberto" and s.get("tipo","").startswith("HEDGE")]
        if not abertos:
            send_telegram("📭 Nenhuma posição HEDGE aberta.", chat_id); return

        # Agrupa por símbolo (par long+short = um hedge)
        por_sym = {}
        for s in abertos:
            por_sym.setdefault(s["symbol"], []).append(s)

        msg = f"🛡️ <b>Posições HEDGE</b> ({len(abertos)} ordens em {len(por_sym)} par(es))\n\n"
        for sym, sinais in por_sym.items():
            price_now = None
            try:
                cm = get_candles(sym, "1m", 2)
                if cm: price_now = cm[-1]["close"]
            except: pass
            msg += f"<b>{sym}</b>" + (f"  (atual: ${price_now:,.4f})\n" if price_now else "\n")
            for s in sinais:
                dir_ = s["direcao"]; acao = "LONG" if dir_=="up" else "SHORT"
                ep, sp, tp, rr = s["entrada"], s["stop"], s["alvo"], s["rr"]
                qty_s = SYMBOLS.get(sym,{}).get("qty", 0)
                lucro_proj = abs(tp-ep)*qty_s
                if price_now:
                    if dir_=="up":
                        progresso = (price_now-ep)/(tp-ep)*100 if tp!=ep else 0
                    else:
                        progresso = (ep-price_now)/(ep-tp)*100 if ep!=tp else 0
                    prog_s = f" · prog: {progresso:+.1f}%"
                else:
                    prog_s = ""
                em = "📈" if dir_=="up" else "📉"
                msg += (f"{em} {acao}  EP:${ep:,.4f}  SL:${sp:,.4f}  TP:${tp:,.4f}  "
                        f"RR 1:{rr}  (≈+${lucro_proj:.2f}){prog_s}\n")
            msg += "\n"
        msg += (f"📐 Critério HEDGE: Risco 1 : Retorno ≥{HEDGE_MIN_RR} · lucro alvo ≥${HEDGE_MIN_PROFIT_USD:.2f}\n"
                f"⚖️ Lote mínimo padrão por perna\n"
                f"🎯 Alvos em estrutura M15/H1/H4 · Stop técnico M15")
        send_telegram(msg, chat_id)
    ...
    # ── RISCO — mostra e ajusta parâmetros de risco ─────────
    elif cmd == "/risco":
        n_open = count_open_trades()
        send_telegram(
            f"⚙️ <b>Gestão de Risco</b>\n\n"
            f"📐 RR mínimo: <b>{MIN_RR}:1</b>\n"
            f"✅ Confirmações TF: <b>{MIN_CONF}</b> (M1+M5)\n"
            f"📂 Trades abertos: <b>{n_open}/{MAX_OPEN_TRADES}</b>\n"
            f"🔒 Máx por símbolo: <b>{MAX_OPEN_PER_SYM}</b>\n"
            f"⏱ Cooldown M1: <b>{M1_COOLDOWN//60}min</b>\n"
            f"⏱ Cooldown M5: <b>{SHORT_COOLDOWN//60}min</b>\n"
            f"⏱ Cooldown HTF: <b>{HTF_COOLDOWN//60}min</b>\n"
            f"💰 Alto Lucro: <b>${HIGH_PROFIT_MIN_USD}-${HIGH_PROFIT_MAX_USD}</b> por trade\n\n"
            f"📊 <b>Matemática:</b>\n"
            f"WR 50% + RR {MIN_RR}:1 = lucro líquido positivo\n"
            f"WR 40% + RR {MIN_RR}:1 = breakeven\n"
            f"WR 40% + RR 1:1 = <b>prejuízo garantido</b>",
            chat_id)

    # ── CALENDÁRIO ECONÔMICO ─────────────────────────────────
    elif cmd in ("/calendario", "/calendar", "/cal"):
        send_telegram("📅 Buscando calendário econômico...", chat_id)
        send_calendar_telegram(chat_id)
...
def count_open_trades():
    """Conta trades atualmente abertos na memória."""
    return len([s for s in memory.get("signals", []) if s.get("status") == "aberto"])

def count_open_trades_sym(sym):
    """Conta trades abertos para um símbolo específico."""
    return len([s for s in memory.get("signals", [])
                if s.get("status") == "aberto" and s.get("symbol") == sym])

def _tf_confirmation(data):
    """
    Retorna (confirmado, n_confirmações, direção_majoritária).
    Exige ao menos MIN_CONF timeframes apontando para a mesma direção.
    M1 conta como 1, M5 conta como 1. Se divergirem → não abre.
    """
    m1t = data.get("m1_trend", "neutral")
    m5t = data.get("m5_trend", "neutral")
    scores = {"up": 0, "down": 0}
    if m1t != "neutral": scores[m1t] += 1
    if m5t != "neutral": scores[m5t] += 1
    best_dir   = max(scores, key=lambda k: scores[k])
    best_score = scores[best_dir]
    confirmed  = best_score >= MIN_CONF
    return confirmed, best_score, best_dir if best_score > 0 else "neutral"

def _entry_matches_direction(entry, direction):
    """Entry só é válido se sua direção bate com a confirmação multi-TF."""
    return entry.get("direcao") == direction
    ...
    while True:
        try:
            ...
            if _loop_n % STATUS_EVERY == 0:
                msg = f"📡 {br_now('%d/%m %H:%M')} UTC\n"
                n_open = count_open_trades()
                msg += f"📂 {n_open}/{MAX_OPEN_TRADES} trades abertos\n"
                for sym, d in results.items():
                    if not d: msg += f"⚠️ {sym}\n"; continue
                    p    = d["price"]
                    m1t  = d["m1_trend"]; m5t = d["m5_trend"]
                    n_m1 = len(d.get("entries_m1",[])); n_m5 = len(d.get("entries_m5",[]))
                    conf, nscore, cdir = _tf_confirmation(d)
                    both = "⚡" if (d.get("both_m1") or d.get("both_m5")) else ""
                    em   = "🟢" if m1t=="up" else ("🔴" if m1t=="down" else
                           "🟢" if m5t=="up" else ("🔴" if m5t=="down" else "⚪"))
                    conf_tag = f"✅{nscore}TF" if conf else f"⏳{nscore}TF"
                    setups = f"M1:{n_m1} M5:{n_m5}{both} {conf_tag}" if (n_m1+n_m5) else f"{m1t[:1].upper()}/{m5t[:1].upper()}"
            ...
            for sym, data in results.items():
                ...
                # ── FILTRO 1: Limite global de trades abertos ──────
                if count_open_trades() >= MAX_OPEN_TRADES:
                    print(f"  [MAX_TRADES] {count_open_trades()}/{MAX_OPEN_TRADES} — aguardando fechamento")
                    break

                # ── FILTRO 2: Limite por símbolo ───────────────────
                if count_open_trades_sym(sym) >= MAX_OPEN_PER_SYM:
                    continue
                ...
                ok_perf, motivo = sym_allowed(sym, _sym_perf)
                if not ok_perf:
                    print(f"  [{sym} bloqueado WR: {motivo}]"); continue

                # ── HEDGE: alvos M15/H1/H4, RR>=HEDGE_MIN_RR, cooldown próprio
                hedge = data.get("hedge")
                if hedge:
                    long_, short_ = hedge
                    ref = long_ or short_
                    key_h = f"{sym}_HEDGE_{round(ref['stop'],4)}_{round(ref['alvo'],4)}"
                    if now_ts - last_signal_time.get(key_h, 0) >= HEDGE_COOLDOWN:
                        fire_signal(data)
                        last_signal_time[key_h] = now_ts
                        continue

                # ── FILTRO 3: Confirmação multi-TF ─────────────────
                # Both (compressão) = dispensa confirmação direcional
                if has_both:
                ...
                # Para M1 e M5: exige confirmação de múltiplos TFs
                conf, nscore, cdir = _tf_confirmation(data)
                if not conf:
                    print(f"  [{sym}] apenas {nscore}TF confirmado — mínimo {MIN_CONF}TF")
                    continue

                # ── M1: só entradas na direção confirmada ──────────
                for e in entries_m1:
                    if not _entry_matches_direction(e, cdir): continue
                    # RR mínimo já garantido no _build_signal, mas valida de novo
                    if e.get("rr", 0) < MIN_RR: continue
                    key = f"{sym}_M1_{e['tipo']}_{round(e['stop'],4)}_{round(e['alvo'],4)}"
                    if now_ts - last_signal_time.get(key, 0) >= M1_COOLDOWN:
                        data_copia = dict(data); data_copia["entry"] = e
                        fire_signal(data_copia)
                        last_signal_time[key] = now_ts
                        break  # 1 entrada M1 por ciclo por símbolo

                # ── M5 Elliott: melhor setup na direção confirmada ──
                best_m5 = data.get("best_m5")
                if best_m5 and _entry_matches_direction(best_m5, cdir):
                    if best_m5.get("rr", 0) >= MIN_RR:
                ...
                # ── HTF: dispara com cooldown maior ────────────────
                for e in data.get("entries_htf_all", []):
                    if not _entry_matches_direction(e, cdir): continue
                    if e.get("rr", 0) < MIN_RR: continue
                ...
                # ── ALTO LUCRO ($1-$5): cooldown 1h ────────────────
                for e in data.get("entries_hp", []):
                    if not _entry_matches_direction(e, cdir): continue
                    key = f"{sym}_HP_{e['tipo']}_{round(e['stop'],4)}"
                    if now_ts - last_signal_time.get(key, 0) >= HIGH_PROFIT_COOLDOWN:
                        data_copia = dict(data); data_copia["entry"] = e
                        # Sinaliza como HP para fire_signal formatar diferente
                        data_copia["hp"] = True
                        fire_signal(data_copia)
                        last_signal_time[key] = now_ts
                        _hp_last_signal[sym]  = now_ts
