...
import requests, time, os, json, base64, threading, hmac, hashlib, uuid
...
last_update_id   = 0
_leverage_set    = set()
_web_replies     = {}   # {"WEB:<uuid>": {"texts":[...], "images":[...base64...]}}
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # ── API: performance/lucro real (BRL) ────────────────
            ...
            try:
                ...
                payload = {
                    "ok": True, "rate": round(rate,4),
                    "deposito_brl": round(dep_total,2),
                    ...
                    "wins": len(wins), "losses": len(losses), "abertos": len(abertos),
                    "wr": round(wr,1), "rnet": round(r_net,2),
                }
            except Exception as e:
        ...
        # ── API: executa comando do Telegram via web ─────────
        elif path == "/cmd":
            qs   = parse_qs(urlparse(self.path).query)
            text = unquote(qs.get("c", [""])[0]).strip()
            if not text:
                self._respond(400, "application/json",
                               json.dumps({"ok": False, "error": "comando vazio"}).encode())
                return
            web_id = f"WEB:{uuid.uuid4().hex}"
            try:
                handle_command(text, web_id)
                result = _web_replies.pop(web_id, {"texts": [], "images": []})
                payload = {"ok": True, "cmd": text, **result}
            except Exception as e:
                payload = {"ok": False, "error": str(e)}
            body = json.dumps(payload, ensure_ascii=False).encode()
            self._respond(200, "application/json", body)
...
def send_telegram(msg, chat_id=None):
    ...
    if isinstance(cid, str) and cid.startswith("WEB:"):
        _web_replies.setdefault(cid, {"texts": [], "images": []})["texts"].append(msg)
        return
    try:
        requests.post(
...
def send_photo_telegram(img_bytes, caption, chat_id=None):
    ...
    if isinstance(cid, str) and cid.startswith("WEB:"):
        b64 = base64.b64encode(img_bytes).decode()
        _web_replies.setdefault(cid, {"texts": [], "images": []})["images"].append(
            {"caption": caption, "data": b64})
        return
    try:
        requests.post(
